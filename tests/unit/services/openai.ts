...
type AIProvider = 'openai' | 'xai' | 'google';
 ...
 * Supports OpenAI, xAI (Grok), and Google (Gemini)
 */
export function createAIClient(apiKey?: string): OpenAI {
  ...
  if (!key || key === 'sk-your-openai-api-key-here') {
    throw new Error(
      'OpenAI API key is required. Please provide an OpenAI API key.'
    );
  }
...
${prompt}
    ...
    // Ollama connection error handling removed
